...
testcase: strict36
thread數量: 1, 4, 12, 24
這裡以下為computation speedup的圖:
![pthread-speedup,strict36](https://hackmd.io/_uploads/ByRukW-Z1e.png)
可以看到各點幾乎都有符合線性增長。
...
process數量: 12
標準差: 0.004405496
![pthread-loadbalance,strict36 ,cpu 2, process12](https://hackmd.io/_uploads/rkCp4W--1e.png)
由此圖和標準差發現，這個程式的工作分配的滿均勻的，標準差小，沒有process的computation time特別多或少。
3. **程式優化比較**:
這裡比較一下有使用AVX512 vectorized優化計算的程式和沒有使用的，看速度差在哪裡。
pthread:
testcase: strict36
thread數量: 1, 4, 12, 24
